...
(defun score (dice)
  (let ((ones 0)
        ...
        (sixes 0))
    (dolist (die dice)
      (case die (1 (incf ones))
                (2 (incf twos))
                (3 (incf threes))
                (4 (incf fours))
                (5 (incf fives))
                (6 (incf sixes))))
    (+ (* 1000 (truncate ones 3)) (* 100 (mod ones 3))
       (* 200 (truncate twos 3))
       (* 300 (truncate threes 3))
       (* 400 (truncate fours 3))
       (* 500 (truncate fives 3)) (* 50 (mod fives 3))
       (* 600 (truncate sixes 3)))))
